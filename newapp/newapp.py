...
class Newapplication(Common):
    ...
    def get_topology_in_namespace_url(self, namespace, name, kind='Deployment'):
        return "v2/kubernetes/clusters/{}/topology/{}?kind={}&name={}".format(self.region_name, namespace, kind, name)

    def get_topology_without_namespace_url(self, kind, name):
        return "v2/kubernetes/clusters/{}/topology/{}/{}".format(self.region_name, kind, name)
    ...
    def get_topology_in_namespace(self, namespace, name, kind='Deployment'):
        logger.info(sys._getframe().f_code.co_name.center(50, '*'))
        url = self.get_topology_in_namespace_url(namespace, name, kind)
        return self.send(method='get', path=url)

    def get_topology_without_namespace(self, kind, name):
        logger.info(sys._getframe().f_code.co_name.center(50, '*'))
        url = self.get_topology_without_namespace_url(kind, name)
